...
import gspread
from gspread.exceptions import SpreadsheetNotFound, WorksheetNotFound, APIError as GSpreadAPIError
...
MSG_MEMBER_JOIN_ERROR = os.getenv(
    "MSG_MEMBER_JOIN_ERROR",
    "⚠️ Hubo un error al procesar tu verificación."
)
...
MSG_VERIFY_SUCCESS = os.getenv(
    "MSG_VERIFY_SUCCESS",
    "✅ Verificado. Se te ha asignado el rol `{rol_nombre}`."
)
...
MSG_TESTCLAVE_USAGE = os.getenv(
    "MSG_TESTCLAVE_USAGE",
    "❌ Uso: `!testclave [clave]`"
)
MSG_TESTCLAVE_KEY_NOT_FOUND = os.getenv(
    "MSG_TESTCLAVE_KEY_NOT_FOUND",
    "❌ Clave no encontrada en la hoja de cálculo."
)
MSG_TESTCLAVE_ERROR = os.getenv(
    "MSG_TESTCLAVE_ERROR",
    "❗ Error al probar la clave: `{e}`"
)
...
MSG_SOPORTE_USAGE = os.getenv(
    "MSG_SOPORTE_USAGE",
    "❌ Debes escribir un mensaje. Uso: `!soporte [mensaje]`"
)
...
MSG_SOPORTE_ERROR = os.getenv(
    "MSG_SOPORTE_ERROR",
    "❗ Hubo un error al enviar tu solicitud. Inténtalo más tarde."
)
...
async def on_member_join(member):
    try:
        try:
            ...
            if log_channel: await log_channel.send(f"⚠️ No se pudo enviar DM de bienvenida a {member.mention} ({member.id}). DMs desactivados/bloqueado.")
            return # Cannot proceed if welcome DM fails
        except DiscordHTTPException as e_discord_http:
            if log_channel: await log_channel.send(f"❗ Error API Discord enviando DM de bienvenida a {member.mention} ({member.id}): {e_discord_http}")
        ...
        while attempts_left > 0:
            ...
            try:
                ...
                if VERIFICATION_MAX_ATTEMPTS - attempts_left > 0 : # Not the first attempt
                     timeout_message_to_send = MSG_MEMBER_JOIN_TIMEOUT + " (Intento agotado)"

                try: await member.send(timeout_message_to_send)
                except (DiscordForbiddenError, DiscordHTTPException): pass # User might have blocked/left
                
                if log_channel:
                    await log_channel.send(f"⏰ **{member.name} ({member.id})** no introdujo la clave a tiempo (intento {VERIFICATION_MAX_ATTEMPTS - attempts_left + 1}).")
            ...
            if attempts_left > 0:
                try:
                    await member.send(MSG_VERIFY_RETRY_PROMPT.format(attempts_left=attempts_left))
                except (DiscordForbiddenError, DiscordHTTPException):
                     if log_channel: await log_channel.send(f"⚠️ No se pudo enviar DM de reintento a {member.mention} ({member.id}). DMs desactivados/bloqueado.")
                ...
                try:
                    ...
                    if log_channel: await log_channel.send(f"⚠️ No se pudo enviar DM de 'sin intentos' a {member.mention} ({member.id}). DMs desactivados/bloqueado.")
                
                if log_channel:
                    await log_channel.send(f"❌ **{member.name} ({member.id})** falló la verificación después de {VERIFICATION_MAX_ATTEMPTS} intentos.")
    ...
    # Specific error handlers from previous step (DiscordForbiddenError, DiscordHTTPException for on_member_join scope)
        ...
        if log_channel: await log_channel.send(f"❗ **Error de permisos Discord (Forbidden)** en on_member_join para {member.name} ({member.id}): {e_forbidden}. Revisar permisos del bot.")
        try: await member.send(MSG_VERIFY_ERROR_PERMISSION)
        except: pass
    except DiscordHTTPException as e_http: 
        if log_channel: await log_channel.send(f"❗ **Error HTTP Discord** en on_member_join para {member.name} ({member.id}): {e_http}. API podría tener problemas.")
        try: await member.send(MSG_VERIFY_ERROR_DISCORD_API)
        except: pass
        ...
        if log_channel:
            await log_channel.send(f"❗ Error crítico no manejado en on_member_join para **{member.name} ({member.id})**: `{e}`")
        try:
