...
from bot import (
    MSG_VERIFY_SUCCESS, MSG_VERIFY_ROLE_NOT_FOUND,
    MSG_VERIFY_ERROR_CRITICAL_SHEET, MSG_VERIFY_ERROR_SHEET_API,
    MSG_VERIFY_ERROR_PERMISSION, MSG_VERIFY_ERROR_DISCORD_API,
    MSG_VERIFY_ERROR_GENERAL, GOOGLE_SHEET_NAME, GOOGLE_SHEET_TAB # For log messages
)
...
class TestBotSetup(unittest.TestCase): # This can remain TestCase if its tests are synchronous
...
class TestIsBotAdmin(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_member_object_exists_but_no_admin_permissions_attribute(self):
        ...
        result = await is_bot_admin(ctx)
        self.assertFalse(result) # Should not error and return False as admin status is not True
...
class TestVerifyMemberKey(unittest.IsolatedAsyncioTestCase):
    def setUp(self):
        ...
        self.sample_row_data = {"Clave": "VALIDKEY", "Nombre Discord": "VerifiedUser", "Rol Asignado": "VerifiedRole"}
        self.mock_sheet.get_all_records.return_value = [self.sample_row_data]
        
        self.mock_role = MagicMock(name="VerifiedRole_Mock") # Give mock_role a name for easier debugging
    ...
    async def test_valid_key_role_found_all_success(self, mock_discord_utils_get):
        ...
        self.mock_member.send.assert_called_once_with(MSG_VERIFY_SUCCESS.format(rol_nombre="VerifiedRole"))
        ...
        mock_discord_utils_get.assert_called_once_with(self.mock_member.guild.roles, name="VerifiedRole")
    ...
    async def test_valid_key_role_not_found(self, mock_discord_utils_get):
        ...
        self.mock_member.send.assert_called_once_with(MSG_VERIFY_ROLE_NOT_FOUND.format(rol_nombre="VerifiedRole"))
        self.mock_log_channel.send.assert_called_once_with(
    ...
    async def test_member_edit_forbidden(self, mock_discord_utils_get):
        mock_discord_utils_get.return_value = self.mock_role
        self.mock_member.edit.side_effect = DiscordForbiddenError(MagicMock(), "Cannot edit nick")
        
        result = await _verify_member_key(
            self.mock_member, "VALIDKEY", self.mock_sheet, self.mock_log_channel
        )
        self.assertTrue(result) # Partial success
        self.mock_log_channel.send.assert_any_call(f"⚠️ **Permiso denegado:** No se pudo cambiar el apodo de {self.mock_member.mention} ({self.mock_member.id}) a 'VerifiedUser'.")
        self.mock_member.add_roles.assert_called_once_with(self.mock_role) # Should still try to add role

    @patch('bot.discord.utils.get')
    async def test_member_add_roles_forbidden(self, mock_discord_utils_get):
        mock_discord_utils_get.return_value = self.mock_role
        self.mock_member.add_roles.side_effect = DiscordForbiddenError(MagicMock(), "Cannot add role")
        
        result = await _verify_member_key(
            self.mock_member, "VALIDKEY", self.mock_sheet, self.mock_log_channel
        )
        self.assertTrue(result)
        self.mock_log_channel.send.assert_any_call(f"⚠️ **Permiso denegado:** No se pudo asignar el rol 'VerifiedRole' a {self.mock_member.mention} ({self.mock_member.id}).")
        self.mock_member.send.assert_any_call(MSG_VERIFY_ERROR_PERMISSION + " (No se pudo asignar el rol 'VerifiedRole')")

    @patch('bot.discord.utils.get')
    async def test_member_send_fails_after_success(self, mock_discord_utils_get):
        mock_discord_utils_get.return_value = self.mock_role
        # First send (success message) fails
        self.mock_member.send.side_effect = DiscordForbiddenError(MagicMock(), "Cannot send DM")
        ...
        self.assertFalse(result) # Because the DM send error is caught and returns False
    ...
    async def test_log_channel_send_fails(self, mock_discord_utils_get):
        mock_discord_utils_get.return_value = self.mock_role
        self.mock_log_channel.send.side_effect = DiscordHTTPException(MagicMock(), "Log channel send failed")
